...
test("idempotency replays cached response on key match", async () => {
  ...
  const ctx = { db, logger: NOOP_LOGGER } as unknown as HandlerCtx;
  const req1 = new Request("http://x/v1/open_session", { method: "POST", headers: { "Idempotency-Key": "k1", "X-Adapter-Id": "a" } });
  const req2 = new Request("http://x/v1/open_session", { method: "POST", headers: { "Idempotency-Key": "k1", "X-Adapter-Id": "a" } });
...
test("idempotency does not cache non-2xx responses", async () => {
  ...
  const ctx = { db, logger: NOOP_LOGGER } as unknown as HandlerCtx;
  const req = new Request("http://x/v1/open_session", { method: "POST", headers: { "Idempotency-Key": "k2", "X-Adapter-Id": "a" } });
